...
extension DoubleExtention on double {
  double coerceHorizontal(var trackStartX,
      var trackEndX,) {
    return this.coerceAtMost(trackEndX).coerceAtLeast(trackStartX);
  }

  double coerceVertical(var trackY,
      var stretchRange,) {
    return this
  ...
  double coerceToStretchRangeWithPivotAtTheMiddle(var x,
      var stretchRange, var height, var startX, var endX,) {
    if (this < 0) {
      return this.coerceAtLeast(
          x < (endX + startX) / 2 ?
          -(((2 * (stretchRange + height / 2) - height) * (x - startX)) / ((endX + startX) - (2 * startX))) + (0) :
          -(((2 * (stretchRange + height / 2) - height) * (x - endX)) / ((endX + startX) - (2 * endX))) + (0)
      );
    } else {
